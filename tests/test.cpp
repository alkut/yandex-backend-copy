...
TEST(echo_server, url) {
    ...
    std::string resp = ExecuteCurl(query_url, query_params, query_body);
...
TEST(echo_server, params) {
    ...
    std::string query_url = "http://localhost:8080/imports";
    std::unordered_map <std::string, std::string> query_params = {{"a","3"}, {"b","4"}};
    std::string query_body = "";
    std::string resp = ExecuteCurl(query_url, query_params, query_body);
    auto result = nlohmann::json::parse(resp);
...
TEST(echo_server, all) {
    std::string query_route = "/imports";
    std::string query_url = "http://localhost:8080/imports";
    std::unordered_map <std::string, std::string> query_params = {{"mango", "apple"}, {"cucumber", "banana"}};
...
void runtests(int args, char **argv, int& code, std::binary_semaphore & bs) {
    ...
    bs.acquire();
    code = RUN_ALL_TESTS();
    ExecuteCurl("http://localhost:8080/shutdown", {}, "");
}
